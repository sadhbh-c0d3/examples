...
public:
    ...
    [[nodiscard]] Action ProcessBackwards() const override
    {
        ...
        return deferred_coroutine([_self = shared_from_this(), this]() -> Coroutine {
            auto propagate = m_inputPin.read()->PropagateBackwards();
            co_await propagate;
...
public:
    ...
    [[nodiscard]] Action ProcessBackwards() const override
    {
        ...
        return deferred_coroutine([_self = shared_from_this(), this]() -> Coroutine {
            ...
            if (auto propagate = m_inputPin.read()->PropagateBackwards(); propagate.has_value()) {
                co_await propagate;
            }
...
void s10_dynamic_graph_and_coro(ExecutionPolicyConcept auto &&policy)
{
    DBG("TEST: test_s10_dynamic_graph_and_coro - " << policy);

    std::cout << "TEST: test_s10_dynamic_graph_and_coro - " << policy << std::endl;
...
{
    s10_dynamic_graph_and_coro(StandardPolicy<false>{});

    s10_dynamic_graph_and_coro(StandardPolicy<true>{});
}