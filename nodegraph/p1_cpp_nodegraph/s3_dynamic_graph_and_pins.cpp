...
public:
    ...
    std::set<INode *> GetSourceNodes()
    {
        ...
        {
            if (not nodePtr->HasInputs()) { assert(nodePtr->HasOutputs()); sourceNodes.insert(nodePtr.get()); }
        }
    ...
    std::set<INode *> GetTargetNodes()
    {
        ...
        for (auto &nodePtr : m_nodes)
        {
            if (not nodePtr->HasOutputs()) { assert(nodePtr->HasInputs()); targetNodes.insert(nodePtr.get()); }
        }
