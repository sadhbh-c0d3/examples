...
{
    node: Weak<RwLock<dyn GraphNode>>,
    data: T
}

impl<T> GraphNodePin<T> where T: Clone + Default + 'static
{
    pub fn new(node: Weak<RwLock<dyn GraphNode>>) -> Self {
        Self {
            node,
    ...
    pub fn get_node(&self) -> Option<Arc<RwLock<dyn GraphNode>>> {
        self.node.upgrade()
    }
...
{
    pub fn new_rc(node: Weak<RwLock<dyn GraphNode>>) -> Arc<RwLock<Self>> {
        Arc::new_cyclic(|me|{
            RwLock::new(Self {
    ...
    fn get_output_node(&self) -> Option<Arc<RwLock<dyn GraphNode>>> {
    ...
    pub fn propagate_backwards(&mut self) {
        if let Some(node) = self.get_output_node() {
            node.write().unwrap().process_backwards();
        }
    }
}
...
impl<T> GraphNodeOutputPin<T> where T: Clone + Default + 'static
{
    ...
    pub fn propagate_forwards(&mut self) {
        for input in self.get_connections().iter() {
            if let Some(node) = input.read().unwrap().pin.get_node() {
...
{
    pub fn new_rc() -> Arc<RwLock<dyn GraphNode>> {
        Arc::new_cyclic(|me| {
            RwLock::new(Self {
                output_pin: GraphNodeOutputPin::new_rc(me.clone() as Weak<RwLock<dyn GraphNode>>)
            })
        })
    }
...
impl<T> GraphNode<> for SourceGraphNode<T> where T: Clone + Default + 'static
{
    ...
    fn process_forwards(&mut self) {
        let mut output_pin = self.output_pin.write().unwrap();
...
impl<T> TargetGraphNode<T> where T: Clone + Default
{
    pub fn new_rc() -> Arc<RwLock<dyn GraphNode>> {
        Arc::new_cyclic(|me| {
            RwLock::new(Self {
                input_pin: GraphNodeInputPin::new_rc(me.clone() as Weak<RwLock<dyn GraphNode>>)
            })
        })
    }
...
{
    fn process_backwards(&mut self) {
        let mut input_pin =self.input_pin.write().unwrap();
...
where
    ...
    F: Fn(X) -> Y
{
    pub fn new_rc(func: F) -> Arc<RwLock<dyn GraphNode>> {
        Arc::new_cyclic(|me| {
            RwLock::new(Self {
                input_pin: GraphNodeInputPin::new_rc(me.clone() as Weak<RwLock<dyn GraphNode>>),
                output_pin: GraphNodeOutputPin::new_rc(me.clone() as Weak<RwLock<dyn GraphNode>>),
    ...
    F: Fn(X) -> Y
{
    fn process_backwards(&mut self) {
        ...
        self.output_pin.write().unwrap().pin.set_data(result);
    }

    fn process_forwards(&mut self) {
        let mut output_pin = self.output_pin.write().unwrap();
        output_pin.pin.set_data((self.func)(self.input_pin.read().unwrap().pin.get_data().clone()));
        output_pin.send_data();
        output_pin.propagate_forwards();
    }
...
pub struct NodeGraph {
    nodes: Vec<Arc<RwLock<dyn GraphNode>>>
}

impl NodeGraph {
    pub fn new(nodes: Vec<Arc<RwLock<dyn GraphNode>>>) -> Self {
        Self {
            nodes
        }
    }

    pub fn get_matching_nodes<F>(&self, f: F) -> Vec<Arc<RwLock<dyn GraphNode>>> 
        where F: Fn(&&Arc<RwLock<dyn GraphNode>>) -> bool + 'static {
           self.nodes.iter().filter(f).cloned().collect()
    }

    pub fn get_source_nodes(&self) -> Vec<Arc<RwLock<dyn GraphNode>>> {
        self.get_matching_nodes(|node|
            node.read().unwrap().get_inputs().is_none() && node.read().unwrap().get_outputs().is_some())
    }
    
    pub fn get_target_nodes(&self) -> Vec<Arc<RwLock<dyn GraphNode>>> {
        self.get_matching_nodes(|node|
            node.read().unwrap().get_inputs().is_some() && node.read().unwrap().get_outputs().is_none())
    }
    
    pub fn get_transform_nodes(&self) -> Vec<Arc<RwLock<dyn GraphNode>>> {
        self.get_matching_nodes(|node|
            node.read().unwrap().get_inputs().is_some() && node.read().unwrap().get_outputs().is_some())
    }
...
#[cfg(test)]
fn connect_output_and_input_nodes(output_node: &Arc<RwLock<dyn GraphNode>>, input_node: &Arc<RwLock<dyn GraphNode>>)
 -> Result<(), Box<dyn Error + 'static>>
{
    let output_pins= output_node.read().unwrap().get_outputs().ok_or("Cannot get output output")?;
    let input_pins= input_node.read().unwrap().get_inputs().ok_or("Cannot get input input")?;
...
#[cfg(test)]
fn find_matching_transform_node_by_output_data_type<T>(graph: &NodeGraph) -> Option<Arc<RwLock<dyn GraphNode>>>
where T: Clone + Default + 'static {

    graph.get_transform_nodes().iter().find(|node| 
            node.read().unwrap().get_outputs().and_then(|pins| 
                pins.iter().find(|pin| 
...
#[cfg(test)]
fn find_matching_target_node_by_input_data_type<T>(graph: &NodeGraph) -> Option<Arc<RwLock<dyn GraphNode>>>
where T: Clone + Default + 'static {

    graph.get_target_nodes().iter().find(|node| 
        (*node).clone().read().unwrap().downcast_ref::<TargetGraphNode<T>>().is_some()
    ).cloned()
}

#[cfg(test)]
fn connect_some_nodes(graph: &NodeGraph)
 -> Result<(Arc<RwLock<dyn GraphNode>>, Arc<RwLock<dyn GraphNode>>, Arc<RwLock<dyn GraphNode>>),
...
#[cfg(test)]
fn load_example_value(source_node: Arc<RwLock<dyn GraphNode>>, value: ExampleInput)
{
    if let Some(node) = source_node.write().unwrap().downcast_mut::<SourceGraphNode<ExampleInput>>() {
        node.load_value(value);
    }
}

#[cfg(test)]
fn get_example_result<T>(target_node: Arc<RwLock<dyn GraphNode>>) -> Option<T>
where
    T: Default + Clone + 'static
{
    match target_node.write().unwrap().downcast_mut::<TargetGraphNode<T>>() {
...
fn test_me() {
    ...
    source_node.write().unwrap().process_forwards();
    ...
    target_node_u.write().unwrap().process_backwards();
    target_node_v.write().unwrap().process_backwards();
