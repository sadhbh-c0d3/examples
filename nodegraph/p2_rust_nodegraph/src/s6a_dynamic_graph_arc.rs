...
{
    pub fn new_rc(node: Weak<dyn GraphNode>) -> Arc<RwLock<Self>> {
        Arc::new_cyclic(|me|{
            RwLock::new(Self {
    ...
    fn get_output_node(&self) -> Option<Arc<dyn GraphNode>> {
...
impl<T> GraphNode<> for SourceGraphNode<T> where T: Clone + Default + 'static
{
    ...
    fn process_forwards(&self) {
        let guard = self.output_pin.read().unwrap();
        guard.send_data();
...
{
    fn process_backwards(&self) {
        self.input_pin.read().unwrap().propagate_backwards();
        self.input_pin.write().unwrap().receive_data();
    }
    ...
    F: Fn(X) -> Y
{
    ...
    fn send_data(&self, data: Y) {
        let mut pin_write = self.output_pin.write().unwrap();
        pin_write.pin.set_data(data);
        pin_write.send_data();
    }

    fn receive_data(&self) -> X {
        let mut pin_write = self.input_pin.write().unwrap();
    ...
    F: Fn(X) -> Y
{
    ...
    fn process_forwards(&self) {
        let x = self.input_pin.read().unwrap().pin.get_data().clone();
        self.send_data((self.func)(x));
        self.output_pin.read().unwrap().propagate_forwards();
    }
