...
pub struct GraphNodeInputPin<T> where T: Clone + Default + 'static
{
    ...
    connection: RefCell<Option<Weak<GraphNodeOutputPin<T>>>>
}

impl<T> GraphNodeInputPin<T> where T: Clone + Default + 'static
{
    pub fn new_rc(node: Weak<dyn GraphNode>) -> Rc<Self> {
        Rc::new_cyclic(|me|{
            Self {
    ...
    fn connect_internal(&self, output_pin: &GraphNodeOutputPin<T>) {
        self.connection.replace(output_pin.me.clone().into());
    }

    pub fn get_connection(&self) -> Option<Rc<GraphNodeOutputPin<T>>> {
...
impl<T> GraphNodePinBase for GraphNodeInputPin<T> where T: Clone + Default + 'static {
    ...
    fn disconnect(&self) {
        if let Some(output_weak) = self.connection.replace(None) {
...
pub struct GraphNodeOutputPin<T> where T: Clone + Default + 'static 
{
    ...
    connections: RefCell<Vec<Weak<GraphNodeInputPin<T>>>>
}
...
impl<T> GraphNodeOutputPin<T> where T: Clone + Default + 'static
{
    ...
    fn connect_internal(&self, input_pin: &GraphNodeInputPin<T>) {
        self.connections.borrow_mut().push(input_pin.me.clone());
    }

    pub fn disconnect_pin(&self, input_pin: &GraphNodeInputPin<T>) {
        self.connections.borrow_mut().retain(
            |connection| !connection.ptr_eq(&input_pin.me));
    }

    pub fn get_connections(&self) -> Vec<Rc<GraphNodeInputPin<T>>> {
        self.connections.borrow().iter().filter_map(
...
{
    pub output_pin: Rc<GraphNodeOutputPin<T>>
}
...
{
    pub input_pin: Rc<GraphNodeInputPin<T>>
}
...
where
    ...
    F: Fn(X) -> Y + 'static
{
    pub input_pin: Rc<GraphNodeInputPin<X>>,
    pub output_pin: Rc<GraphNodeOutputPin<Y>>,
    pub func: F
}
...
#[cfg(test)]
fn connect_output_and_input_nodes(output_node: &Rc<dyn GraphNode>, input_node: &Rc<dyn GraphNode>) -> Result<(), Box<dyn Error + 'static>>
{
...
where T: Clone + Default + 'static {
    ...
    graph.get_transform_nodes().iter().find(|node| 
            node.get_outputs().and_then(|pins| 
                pins.iter().find(|pin| 
                    (*pin).clone().downcast_rc::<GraphNodeOutputPin<T>>().is_ok()
                ).map(|_| true)
            ).is_some()
        ).cloned()
}
...
where T: Clone + Default + 'static {
    ...
    graph.get_target_nodes().iter().find(|node| 
        (*node).clone().downcast_rc::<TargetGraphNode<T>>().is_ok()
    ).cloned()
}

#[cfg(test)]
fn connect_some_nodes(graph: &NodeGraph) -> Result<(Rc<dyn GraphNode>, Rc<dyn GraphNode>, Rc<dyn GraphNode>), Box<dyn Error + 'static>>{
